...
// Helper function to determine the embed color based on the status
const getEmbedColor = (status) => {
    ...
    // Defaults to white if the status is unknown
...
const log = (message, error = false) => {
    ...
    const setChannelName = async (status) => {
        ...
        for (const channelId of STATUS_CHANNELS) {
            try {
                ...
                if (channel.name !== newName) {
                    ...
                    log(`Channel ${chalk.yellowBright(channel.name)} (${channelId}) name updated to: ${newName} (Guild: ${chalk.cyanBright(channel.guild.name)})`);
                }
                await sleep(1000);
            } catch (error) {
        ...
        for (const [guildId, config] of guildConfigs.entries()) {
            ...
            try {
                ...
                if (channel.name !== newName) {
                    await channel.setName(newName);
                    log(`Guild-specific channel ${chalk.yellowBright(channel.name)} (${config.statusChannelId}) name updated to: ${newName} (Guild: ${chalk.cyanBright(channel.guild.name)})`);
                }
                await sleep(1000);
            } catch (error) {
    ...
    for (const channelId of NOTIFICATION_CHANNELS) {
        try {
            ...
            log(`Notification sent to channel ${chalk.yellowBright(channel.name)} (${channelId})`);
            await sleep(1000);
        } catch (error) {
    ...
    for (const [guildId, config] of guildConfigs.entries()) {
        ...
        for (const channelId of config.notificationChannelIds) {
            try {
                ...
                await channel.send({ embeds: [embed] });
                log(`Notification sent to guild-specific channel ${chalk.yellowBright(channel.name)} (${channelId}) (Guild: ${chalk.cyanBright(channel.guild.name)})`);
                await sleep(1000);
            } catch (error) {
    ...
    client.once('ready', async () => {
        ...
        for (const [guildId, config] of guildConfigs.entries()) {
            ...
            if (config.statusChannelId) {
                try {
                    const channel = await client.channels.fetch(config.statusChannelId);
                    log(`[√] Guild status channel ${chalk.yellowBright(channel.name)} (${config.statusChannelId}) validated for Guild ${chalk.cyanBright(channel.guild.name)}`);
                } catch (error) {
                    log(`[×] Invalid guild status channel ${config.statusChannelId} for Guild ${chalk.cyanBright(guildId)}: ${error.message}`, true);
                }
            }

            // Validate multiple notification channels for each guild
            if (config.notificationChannelIds && config.notificationChannelIds.length > 0) {
                for (const channelId of config.notificationChannelIds) {
                    try {
                        const channel = await client.channels.fetch(channelId);
                        log(`[√] Guild notification channel ${chalk.yellowBright(channel.name)} (${channelId}) validated for Guild ${chalk.cyanBright(channel.guild.name)}`);
                    } catch (error) {
        ...
        setInterval(async () => {
            try {
                ...
                fetchInterval = FETCH_INTERVAL_RED; // Use red interval on error
            }
        }, fetchInterval);
    });
