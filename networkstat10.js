...
const getStatusName = (status, guildId = null) => {
    // Check for guild-specific status name
...
const setChannelName = async (status) => {
    ...
    for (const channelId of STATUS_CHANNELS) {
        try {
            ...
            if (channel.name !== newName) {
                ...
                log(`Channel ${channelId} name updated to: ${newName} (Guild: ${channel.guild.id})`);
            }
            await sleep(1000);
        } catch (error) {
    ...
    for (const [guildId, config] of guildConfigs.entries()) {
        ...
        try {
            ...
            if (channel.name !== newName) {
                await channel.setName(newName);
                log(`Guild-specific channel ${config.statusChannelId} name updated to: ${newName} (Guild: ${guildId})`);
            }
            await sleep(1000);
        } catch (error) {
    ...
    for (const [guildId, config] of guildConfigs.entries()) {
        ...
        for (const channelId of config.notificationChannelIds) {
            try {
                ...
                await channel.send(finalMessage);
                log(`Notification sent to guild-specific channel ${channelId} (Guild: ${guildId})`);
                await sleep(1000);
            } catch (error) {
...
const checkStatus = async () => {
    ...
    logStateChange('STATUS_CHECK', {
        ...
        'Recent Notification': recentNotification,
        ...
        if (recentNotification !== lastSentNotification) {
            ...
            notificationTimeout = setTimeout(async () => {
                if (recentNotification && recentNotification !== lastSentNotification) {
                    ...
                    log(`Last sent notification updated to: ${recentNotification}`);
...
client.once('ready', async () => {
    ...
    for (const [guildId, config] of guildConfigs.entries()) {
        log(`Validating channels for Guild ${guildId}...`);
        
        if (config.statusChannelId) {
            try {
                const channel = await client.channels.fetch(config.statusChannelId);
                log(`✓ Guild status channel ${channel.name} (${config.statusChannelId}) validated for Guild ${guildId}`);
            } catch (error) {
                log(`✗ Invalid guild status channel ${config.statusChannelId} for Guild ${guildId}: ${error.message}`, true);
            }
        }
        ...
        if (config.notificationChannelIds && config.notificationChannelIds.length > 0) {
            for (const channelId of config.notificationChannelIds) {
                try {
                    const channel = await client.channels.fetch(channelId);
                    log(`✓ Guild notification channel ${channel.name} (${channelId}) validated for Guild ${guildId}`);
                } catch (error) {
                    log(`✗ Invalid guild notification channel ${channelId} for Guild ${guildId}: ${error.message}`, true);
                }
            }
        }
    }
